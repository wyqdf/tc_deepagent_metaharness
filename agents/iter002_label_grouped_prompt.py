"""BM25 retrieval with label-grouped prompt organization.

Mechanism change vs iter001_label_coverage_bm25: Same retrieval and selection
logic, but changes PROMPT ORGANIZATION. Instead of listing examples in flat
relevance order (Q/A, Q/A, ...), groups them by label with headers so the
model can compare within-class patterns and across-class differences at a glance.

This addresses a key weakness: in flat listing, the model must mentally group
examples by label to compare. Explicit grouping reduces cognitive load and
makes distinguishing features more salient.
"""
...
PROMPT_TEMPLATE = """Classify the problem below. Examples are grouped by answer for comparison.
...
**Instructions:**
- Compare the problem against each answer group above.
- Pick the answer whose group's examples most closely match the problem.
...
class LabelGroupedPrompt(BaseAgentMemory):
    """BM25 retrieval with label-grouped prompt organization."""
    ...
    def _format_examples(self, query: str) -> str:
        ...
        if not idxs:
            return ""
        # Group by label, preserving retrieval-rank order within each group
        groups: dict[str, list[str]] = defaultdict(list)
        # Track order of first appearance for stable group ordering
        ...
        for i in idxs:
            ...
            label = ex["target"]
            entry = f"  - {q}"
            if total + len(entry) + len(label) + 20 > MAX_CHARS:
                break
