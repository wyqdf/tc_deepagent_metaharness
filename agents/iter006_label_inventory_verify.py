"""Label inventory + confusion-guided two-pass verification.

Mechanism change vs iter005_confusion_verify (base): Adds a LABEL INVENTORY
section to the pass-1 prompt that lists all known target labels from training.
This constrains the model to use exact label names instead of inventing
approximate ones.

Analysis of LawBench errors shows the model frequently predicts labels that
are close but not exact (e.g., "侮辱罪" instead of "侮辱", "受贿" instead of
"非国家工作人员受贿", "伪造金融票证" instead of "伪造、变造金融票证").
A label inventory gives the model the exact vocabulary to choose from.

This is a PROMPT ORGANIZATION change (adding structured label knowledge)
combined with the proven two-pass confusion-guided verification.
"""
...
**Known answer labels (use EXACT wording from this list):**
{label_inventory}
...
**Instructions:**
- You MUST choose your answer from the known labels listed above. Use the exact wording.
...
- If multiple labels apply, separate them with semicolons.
- Respond in JSON format.

{{"reasoning": "[your reasoning]", "final_answer": "[your answer]"}}"""

PASS1_NO_INVENTORY = """Solve the problem below based on the examples provided.
...
CONFUSION_MIN = 1
LABEL_INVENTORY_MAX = 80  # max labels to show in inventory
...
class LabelInventoryVerify(BaseAgentMemory):
    """Label inventory + confusion-guided two-pass verification."""
    ...
    def _build_label_inventory(self) -> str:
        """Build a compact label inventory string."""
        if not self._all_labels or len(self._all_labels) > LABEL_INVENTORY_MAX:
            return ""
        sorted_labels = sorted(self._all_labels)
    ...
    def predict(self, input: str) -> tuple[str, dict[str, Any]]:
        ...
        # Build pass-1 prompt with or without label inventory
        label_inventory = self._build_label_inventory()
        if label_inventory and idxs:
            ...
            prompt1 = PASS1_NO_INVENTORY.format(
                examples_section=examples_section, input=input
            )
        else:
            prompt1 = PASS1_NO_INVENTORY.format(examples_section="", input=input)
    ...
    def learn_from_batch(self, batch_results: list[dict[str, Any]]) -> None:
        for r in batch_results:
            ...
            # Track all labels
            self._all_labels.add(str(r["ground_truth"]))
    ...
    def get_state(self) -> str:
        return json.dumps({
            ...
            "all_labels": sorted(self._all_labels),
        }, ensure_ascii=False)
    ...
    def set_state(self, state: str) -> None:
        ...
        self._all_labels = set(data.get("all_labels", []))
        if not self._all_labels:
            self._all_labels = {ex["target"] for ex in self.examples if "target" in ex}
        self._index_dirty = True
