"""Label-snap post-processing on top of confusion-guided two-pass verify.

Mechanism change vs iter005_confusion_verify (base): Adds a LABEL SNAP
post-processing step after both prediction passes. During training, collects
all unique target labels seen. After prediction, if the output doesn't exactly
match any known label, maps it to the nearest known label using character-bigram
Jaccard similarity.

This addresses a key error mode on LawBench where the model predicts near-miss
label names (e.g., "非法采伐、毁坏国家重点保护植物罪" instead of
"非法采伐、毁坏国家重点保护植物", or invents labels not in the training set).
For multi-label predictions (containing ';'), each component is snapped
independently.

On single-label datasets (S2D, USPTO), predictions that already match known
labels pass through unchanged. On USPTO (SMILES strings), the snap is unlikely
to fire since predictions are generated from examples, not from a fixed label set.

Axes changed: POST-PROCESSING / VERIFICATION — new label-constraint mechanism
that operates after the existing two-pass predict+verify pipeline.
"""
...
class LabelSnapVerify(BaseAgentMemory):
    """Two-pass confusion verify + label-snap post-processing."""
    ...
    def __init__(self, llm):
        ...
        # Label snap: all unique labels seen in training
        self._known_labels: set[str] = set()
        # Track if task appears to have multi-label targets
    ...
    def _snap_label(self, prediction: str) -> str:
        """Snap prediction to nearest known label(s)."""
        if not self._known_labels:
            return prediction
        # For multi-label, split and snap each component
    ...
    def _snap_single(self, pred: str) -> str:
        """Snap a single label to nearest known label."""
        if pred in self._known_labels:
            return pred
        # Find nearest by character bigram Jaccard
        best_label = pred
        best_sim = 0.0
    ...
    def predict(self, input: str) -> tuple[str, dict[str, Any]]:
        ...
        final_answer = final_answer or prediction
        # Apply label snap to final answer
        final_answer = self._snap_label(final_answer)
        ...
        return final_answer, {
            ...
            "snapped": final_answer != (extract_json_field(response2, "final_answer") or prediction),
        }
    ...
    def learn_from_batch(self, batch_results: list[dict[str, Any]]) -> None:
        for r in batch_results:
            ...
            # Track known labels (split multi-label targets)
            if ';' in target:
