"""MMR-diversified retrieval with confusion-guided verification.

Mechanism change vs iter005_confusion_verify (base): Replaces the label-coverage
selection heuristic with MMR (Maximal Marginal Relevance) diversity selection.

Current iter005 uses a 3-stage selection: anchors -> label-coverage -> fill.
This works well but can include redundant examples (same label, very similar
content) that waste prompt budget. MMR explicitly penalizes redundancy:

  MMR(d) = lambda * BM25(q, d) - (1-lambda) * max_sim(d, selected)

where max_sim uses Jaccard token overlap between documents. This ensures every
selected example adds genuinely new information.

Key differences from existing systems:
- iter005_confusion_verify: Uses label-coverage heuristic for diversity.
  MMR provides content-level diversity within AND across labels.
- iter003_contrastive_retrieval: Adds contrastive partners after selection.
  MMR integrates diversity INTO the selection process.
- iter008_discrim_token_verify: Changes the BM25 index. MMR changes the
  selection algorithm on the same index.

The verify pass remains confusion-guided (proven best). Only the pass1
retrieval quality changes.

On LawBench (many labels, similar crime descriptions), MMR should surface
more diverse crime types in the prompt instead of showing 3 variations of
the same crime. On S2D (medical symptoms), MMR should surface different
diseases instead of multiple examples of the most similar disease.
On USPTO (unique targets), MMR degrades gracefully to pure BM25 since
each example has unique content.
"""
...
class MmrConfusionVerify(BaseAgentMemory):
    """MMR-diversified retrieval + confusion-guided verification."""
    ...
    def _mmr_select(self, query: str) -> list[int]:
        """Select examples using MMR for content diversity."""
        self._ensure_index()
        ...
        # Get top CANDIDATE_POOL by BM25
        pool_indices = sorted(range(n), key=lambda i: -bm25_scores[i])[:CANDIDATE_POOL]

        if not pool_indices:
            return []

        # Normalize BM25 scores within pool for MMR
